...
class TestBulkMailView(BulkMailTestCase):
    ...
    def test_get_all_mails_unauthorized_exc(self):
        response = self.client.get(
            f"{reverse('view_all_bulk_mails')}?{urlencode({'page': 1, 'page_size': 1})}"
        )
        response_data = response.json()
    ...
    def test_get_mail(self):
        ...
        response = self.client.get(
            reverse(
                "get_bulk_mail",
                kwargs={"mail_id": self.bulk_mail_model.id},
            ),
            headers=self.headers,
        )
        response_data = response.json()
    ...
    def test_send_mail(self):
        ...
        response = self.client.post(
            reverse("send_bulk_mail"),
            data=self.bulk_mail_test_data.send_mail(
    ...
    def test_send_mail_invalid_data_exc(self):
        response = self.client.post(
            reverse("send_bulk_mail"),
            data=self.bulk_mail_test_data.send_mail(sender="invalid"),
            format=self.data_format,
            headers=self.headers,
        )
        response_data = response.json()
    ...
    def test_send_mail_with_template(self):
        self.jwt_decode.return_value = self.mock_decode_token()
        response = self.client.post(
            reverse("send_template_bulk_mail"),
            data=self.bulk_mail_test_data.send_email_with_template(
                sender=self.mail_account_model.mail_address,
    ...
    def test_delete_mail(self):
        ...
        response = self.client.delete(
            reverse("delete_bulk_mail", kwargs={"mail_id": self.bulk_mail_model.id}),
            format=self.data_format,
            headers=self.headers,
        )
